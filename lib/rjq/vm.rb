...
module Rjq
  class VM
    ...
    def binary_stream(op, blocks, input, context)
      ...
      right_values = replayable_stream(each_block(blocks[1].instructions, input, context))
      flat_map_stream(each_block(blocks[0].instructions, input, context)) do |left|
        map_stream(right_values.call) { |right| apply_binary(op, left, right) }
      end
    end
    ...
    def execute_index_filter(values, block, input, context)
      indices = execute_filter(block, input, context)
      values.flat_map { |value| indices.map { |index| read_index(value, index) } }
    end
    ...
    def execute_pipe(left_values, block, context)
      out = []
      left_values.each do |value|
        out.concat(execute_filter(block, value, context))
      rescue ErrorValue => e
        raise ErrorValue.new(e.value, outputs: out + Array(e.outputs))
      rescue BreakSignal => e
    ...
    def execute_append(left_values, block, input, context)
      left_values + execute_filter(block, input, context)
    rescue ErrorValue => e
      raise ErrorValue.new(e.value, outputs: left_values + Array(e.outputs))
    rescue BreakSignal => e
      raise BreakSignal.new(e.label, e.value, outputs: left_values + Array(e.outputs))
    end

    def execute_binding(spec, input, context)
      execute_filter(spec.fetch(:source), input, context).flat_map do |value|
        execute_filter(spec.fetch(:body), input, AST.bind_pattern(context, spec.fetch(:pattern), value))
      end
    end

    def execute_object(pairs, input, context)
      objects = [{}]
      pairs.each do |pair|
        key_values = object_key_outputs(pair.fetch(:key), input, context)
        value_values = execute_filter(pair.fetch(:value), input, context)
    ...
    def object_key_outputs(key, input, context)
      return [key.fetch(:value)] if key.fetch(:type) == :literal

      execute_filter(key.fetch(:block), input, context).map { |value| object_key(value) }
    end
    ...
    def execute_branch(instruction, input, context)
      then_block, else_block = instruction.arg2
      execute_filter(instruction.arg1, input, context).flat_map do |value|
        execute_filter(Value.truthy?(value) ? then_block : else_block, input, context)
      end
    end
    ...
    def execute_try(spec, input, context)
      ...
      spec[:handler] ? execute_filter(spec.fetch(:handler), e.message, context) : []
    end
    ...
    def execute_label(label, block, input, context)
      execute_filter(block, input, context)
    rescue BreakSignal => e
      raise unless e.label == label
    ...
    def execute_unary(op, block, input, context)
      execute_filter(block, input, context).map do |value|
        case op
        when '-'
          unless value.is_a?(Numeric)
            raise TypeError,
                  "#{Value.type_of(value)} (#{short_dump(value)}) cannot be negated"
          end
    ...
    def execute_binary(op, blocks, input, context)
      return execute_alternative(blocks, input, context) if op == '//'
      return execute_boolean(op, blocks, input, context) if %w[and or].include?(op)

      left_values = execute_filter(blocks[0], input, context)
      right_values = execute_filter(blocks[1], input, context)
      left_values.flat_map { |left| right_values.map { |right| apply_binary(op, left, right) } }
    end

    def execute_alternative(blocks, input, context)
      left_values = execute_filter(blocks[0], input, context).select { |value| Value.truthy?(value) }
      return left_values unless left_values.empty?

      execute_filter(blocks[1], input, context)
    end
    ...
    def modulo_values(left, right)
      left_number, right_number = numeric_pair(numeric(left), numeric(right))
      raise TypeError, division_by_zero_message(left_number, right_number, 'divided (remainder)') if right_number.zero?
      if left_number.respond_to?(:infinite?) && left_number.infinite? && right_number.respond_to?(:infinite?) && right_number.infinite?
        return (left_number.negative? ? -1 : 0)
      end
      return 0 if left_number.respond_to?(:infinite?) && left_number.infinite?

      left_number.remainder(right_number)
    end
