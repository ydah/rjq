...
module DifferentialCompat
  ...
  CASES = [
    ...
    Case.new(name: 'deep tail recursion',
             filter: 'def count: if . > 0 then . - 1 | count else . end; 5000 | count',
             input: '', flags: ['-n']),
    Case.new(name: 'branching tail recursion',
             filter: 'def walk: if . > 0 then (. - 1, . - 2) | walk else . end; 3 | walk',
             input: '', flags: ['-n']),
    ...
    Case.new(name: 'nonlinear binding path validation',
             filter: 'path(. as [$x,$y]|empty)', input: '[1,2]', flags: []),
    Case.new(name: 'typed binding alternative fallback',
             filter: 'path(. as [$x]?//{b:$y}|$y)', input: '[1,2]', flags: []),
    Case.new(name: 'binding alternative partial error',
             filter: 'path(. as {a:$x}?//{b:$y}|$y,$x)', input: '{"a":[1,2],"b":3}', flags: []),
    Case.new(name: 'binding variable fallback root',
             filter: 'path(. as {a:$x}?//$y|.)', input: '{"a":1}', flags: []),
    Case.new(name: 'binding partial runtime path',
             filter: 'path(. as $x|$x,.a)', input: '[1,2]', flags: []),
    Case.new(name: 'nested binding path provenance',
             filter: 'path(. as {a:$x}|$x as {b:$y}|$x)', input: '{"a":{"b":1}}', flags: []),
    Case.new(name: 'three binding alternatives over array',
             filter: 'path(. as {a:$x}?//{b:$y}?//[$z]|$z,$y,$x)', input: '[1,2]', flags: []),
    Case.new(name: 'three binding alternatives with root fallback',
             filter: 'path(. as [$x]?//{a:$y}?//$z|$z,$y,$x)', input: '[1,2]', flags: []),
    Case.new(name: 'three binding alternatives over object',
             filter: 'path(. as {a:$x}?//[$y]?//{b:$z}|$z,$y,$x)',
             input: '{"a":[1,2],"b":3}', flags: []),
    Case.new(name: 'binding path diagnostic side effects execute once',
             filter: 'path(. as $x | debug)', input: '', flags: ['-n']),
    Case.new(name: 'all predicate preserves generated false',
             filter: '[all(.[]; (true,false)), all(.[]; empty)]', input: '[1]'),
    Case.new(name: 'any and all short circuit generated errors',
             filter: '[any((true,error("late"))), all((false,error("late")))]', input: '[0]'),
    Case.new(name: 'walk preserves array child generators',
             filter: 'walk((.,.))', input: '[1]'),
    Case.new(name: 'walk array child partial error',
             filter: 'walk((.,error("late")))', input: '[1]'),
    Case.new(name: 'walk object child partial error',
             filter: 'walk((.,error("late")))', input: '{"a":1}'),
    Case.new(name: 'walk object child takes first lazily',
             filter: 'walk(if type=="number" and .==1 then (.,error("late")) else . end)',
             input: '{"a":1,"b":2}'),
    Case.new(name: 'walk preserves output before halt',
             filter: 'walk((.,halt))', input: '1'),
    Case.new(name: 'from entries rejects numeric keys',
             filter: 'from_entries', input: '[{"key":0,"value":1}]'),
    Case.new(name: 'paths preserves predicate multiplicity',
             filter: 'paths((true,true))', input: '[1]'),
    Case.new(name: 'paths evaluates predicate on suppressed root',
             filter: 'paths((true,error("root")))', input: '1'),
    Case.new(name: 'select preserves output before predicate error',
             filter: 'select((true,error("late")))', input: '[1]'),
    Case.new(name: 'map values takes first update lazily',
             filter: 'map_values((.,error("late")))', input: '[1]'),
    Case.new(name: 'map values preserves falsey updates',
             filter: '[map_values(null),map_values(false)]', input: '[1,2]'),
    Case.new(name: 'map constructor hides partial child values',
             filter: 'map((.,error("late")))', input: '[1]'),
    Case.new(name: 'with entries hides partial child values',
             filter: 'with_entries((.,error("late")))', input: '{"a":1}'),
    Case.new(name: 'sort key hides partial values',
             filter: 'sort_by((.,error("late")))', input: '[1,2]'),
    Case.new(name: 'last hides partial argument values',
             filter: 'last((1,error("late")))', input: '', flags: ['-n']),
    Case.new(name: 'IN short circuits source errors',
             filter: 'IN((1,error("late"));.)', input: '1'),
    Case.new(name: 'limit streams before source errors',
             filter: 'limit(2;(1,error("late")))', input: '', flags: ['-n']),
    Case.new(name: 'until streams condition branches',
             filter: 'until((true,error("late"));.)', input: '0'),
    Case.new(name: 'while streams condition branches',
             filter: 'while((true,error("late"));empty)', input: '0'),
    Case.new(name: 'builtin argument Cartesian order',
             filter: 'pow((2,3);(4,5))', input: '', flags: ['-n']),
    Case.new(name: 'jq-defined builtin input argument order',
             filter: 'range((0,input);(2,input))', input: '10 20 30 40', flags: ['-n']),
    Case.new(name: 'jq-defined regex argument order',
             filter: 'scan(("a",("b"|debug));("",("i"|debug)))', input: '"ab"'),
    Case.new(name: 'sub argument evaluation order',
             filter: 'sub(("a","b");("x","y");("","g"))', input: '"ab"'),
    Case.new(name: 'ordinary builtin argument partial output',
             filter: 'index((1,error("late")))', input: '[1,2]'),
    Case.new(name: 'index family rejects object needles',
             filter: '[index({}),rindex({}),indices({})]', input: '{}'),
    Case.new(name: 'index family rejects scalar inputs',
             filter: '[index(1),rindex(1),indices(1)]', input: '1'),
    Case.new(name: 'index family empty array needle',
             filter: '[index([]),rindex([]),indices([])]', input: '[0,1]'),
    Case.new(name: 'generated invalid setpath argument',
             filter: 'setpath((["a"],input);(1,input))', input: '10 20'),
    Case.new(name: 'flatten string depth on empty object', filter: 'flatten("x")', input: '{}'),
    Case.new(name: 'flatten explicit null depth', filter: 'flatten(null)', input: '[]'),
    Case.new(name: 'invalid object has key', filter: 'has([])', input: '{}'),
    Case.new(name: 'fractional array has indices',
             filter: '[has(-0.1),has(-0.9),has(-1),has(-1.1),has(1.1),has(nan),has(infinite)]', input: '[0,1]'),
    Case.new(name: 'fractional path index semantics',
             filter: '[getpath([-0.1]),getpath([-1.1]),setpath([-0.1];9),setpath([-1.1];9),' \
                     'delpaths([[-0.1]]),delpaths([[-1.1]]),delpaths([[1.9]])]', input: '[0,1,2]'),
    Case.new(name: 'negative fractional slice endpoint',
             filter: '[.[0:-0.1],.[-0.1:-0.1]]', input: '[0,1,2]'),
    ...
    Case.new(name: 'split generated flags partial error',
             filter: 'split("a";("",error("late")))', input: '"ab"'),
    Case.new(name: 'path partial output is emitted once',
             filter: 'path((.a,error("late")))', input: '{"a":1}'),
    Case.new(name: 'delete hides partial path values',
             filter: 'del((.a,error("late")))', input: '{"a":1}'),
    Case.new(name: 'fromstream yields completed roots before errors',
             filter: 'fromstream(([[],1],error("late")))', input: '', flags: ['-n']),
